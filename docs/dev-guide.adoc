...
* Outputs optimized `DefaultEngineModel`
...
*Guarantee*: Identical input rules → byte-for-byte identical `DefaultEngineModel`.
...
*DefaultRuleCompiler* — compiles JSON rules into optimized `DefaultEngineModel`.
...
*DefaultEngineModel* — immutable compiled rule model.
