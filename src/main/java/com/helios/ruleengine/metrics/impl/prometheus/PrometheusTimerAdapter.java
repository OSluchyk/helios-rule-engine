 ...
 *
 * <p>Prometheus uses histograms for timing measurements, not a dedicated timer type.
 ...
 * <p>Histograms allow Prometheus server to calculate percentiles (P50, P95, P99)
 * efficiently using the <code>histogram_quantile()</code> function. Client-side
 * percentile calculation would require storing all measurements, which doesn't scale.
 *
 * <h3>Bucket Configuration:</h3>
 * <p>Default buckets: 1ms, 5ms, 10ms, 50ms, 100ms, 500ms, 1s, 5s
 * <pre>{@code
 ...
 *
 * <h3>Thread Safety:</h3>
 * <p>All operations are thread-safe. Multiple threads can call {@link #record(Callable)}
 * and {@link #record(Duration)} concurrently.
 *
 * <h3>Important Limitation:</h3>
 * <p>{@link #percentile(double)} is NOT supported. Percentiles are calculated by
...
final class PrometheusTimerAdapter implements Timer {
     ...
     *
     * <p>The timing starts immediately before calling {@code callable.call()}
     * and stops when it returns or throws an exception. The duration is recorded
     * even if the callable throws.
     *
     * <p><b>Thread-safe:</b> Multiple threads can time operations concurrently.
     *
     * <p><b>Example:</b>
     * <pre>{@code
     ...
     *
     * <p>Use this when timing happens externally or when migrating from
     * manual timing code.
     *
     * <p><b>Important:</b> Duration is converted to seconds before recording,
     * as Prometheus convention is to use base units (seconds, not milliseconds).
     *
     * <p><b>Thread-safe:</b> Multiple threads can record concurrently.
     *
     * <p><b>Example:</b>
     * <pre>{@code
     ...
     *
     * <p>Prometheus histograms do not calculate percentiles client-side.
     ...
     * </ol>
     *
     * <p><b>Alternative:</b> For testing, use {@link com.helios.rules.metrics.testing.InMemoryTimer}
     ...
     *
     * <p>This is equivalent to the <code>_count</code> metric exposed by Prometheus:
     * <pre>
     * metric_name_count 42.0
     * </pre>
     *
     * <p><b>Thread-safe:</b> Read may not reflect concurrent recordings happening
     ...
     *
     * <p>This is equivalent to the <code>_sum</code> metric exposed by Prometheus:
     * <pre>
     * metric_name_sum 12.345
     * </pre>
     *
     * <p>To calculate average latency:
     * <pre>{@code
     * double avgSeconds = timer.sum() / timer.count();
     * }</pre>
     *
     * <p><b>Thread-safe:</b> Read may not reflect concurrent recordings.
     *
     ...
     *
     * <p><b>Warning:</b> Direct manipulation bypasses adapter logic.
